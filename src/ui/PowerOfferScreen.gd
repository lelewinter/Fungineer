...
func _build_frame() -> void:
	...
	panel.position = Vector2(800.0 * 0.5 - 180, 600.0 * 0.5 - 150)
	add_child(panel)

	var title := Label.new()
	title.text = "ESCOLHA UM PODER"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.position = Vector2(800.0 * 0.5 - 160, 600.0 * 0.5 - 138)
...
func show_offer(power_options: Array) -> void:
	...
	var start_y: float = 600.0 * 0.5 - 108.0
	for i in power_options.size():
		...
		btn.position = Vector2(800.0 * 0.5 - 160, start_y + i * 66.0)
		var power_ref = p
