...
const _C_BG: Color       = Color(0.02, 0.02, 0.04)
const _C_FLOOR: Color    = Color(0.07, 0.06, 0.10)
const _C_ENTRY: Color    = Color(0.08, 0.09, 0.22)
const _C_EXIT: Color     = Color(0.07, 0.18, 0.09)
const _C_BORDER: Color   = Color(0.30, 0.25, 0.40, 0.70)
const _C_WALL_CLOSED: Color  = Color(0.22, 0.18, 0.30)
const _C_WALL_CLOSING: Color = Color(0.75, 0.18, 0.08)
const _C_WALL_OPEN: Color    = Color(0.10, 0.10, 0.16, 0.40)
const _C_WALL_OPENING: Color = Color(0.15, 0.70, 0.25)
...
class _MazeHUD:
	...
	func setup(parent: Node) -> void:
		...
		_bag_lbl.modulate = Color(0.95, 0.75, 0.30)
		_layer.add_child(_bag_lbl)

	func refresh(hp: int, bag: int, cap: int) -> void:
		_hp_lbl.text = "HP: %d / %d" % [hp, GameConfig.MAZE_PLAYER_HP]
		_bag_lbl.text = "Mochila: %d / %d" % [bag, cap]
...
func _draw_rooms() -> void:
	for i in _room_rects.size():
		...
		if is_exit:
			...
			floor_c = _C_FLOOR
		draw_rect(r, floor_c)
		draw_rect(r, _C_BORDER, false, 1.5)

	# Zone labels
	var ec := _room_center(_ENTRY_COL, _ENTRY_ROW)
	draw_string(ThemeDB.fallback_font, ec + Vector2(0.0, -30.0), "ENTRADA",
		HORIZONTAL_ALIGNMENT_CENTER, -1, 11, Color(0.60, 0.70, 1.00, 0.85))
...
func _draw_walls() -> void:
	for wall in _walls:
		...
		match wall.state:
			_DynWall.State.OPEN:
				draw_rect(r, _C_WALL_OPEN)
				_draw_timer_arc(r, remaining, wall.phase_dur, Color(0.30, 0.80, 0.30, 0.60))
			_DynWall.State.CLOSING:
				draw_rect(r, Color(_C_WALL_CLOSING.r, _C_WALL_CLOSING.g, _C_WALL_CLOSING.b,
					0.50 + progress * 0.50))
				var pulse := 0.5 + 0.5 * sin(Time.get_ticks_msec() * 0.008)
				draw_rect(r, Color(1.0, 0.40, 0.10, 0.85 * pulse), false, 2.5)
				_draw_timer_arc(r, remaining, wall.phase_dur, Color(1.00, 0.20, 0.10, 0.95))
			_DynWall.State.CLOSED:
				draw_rect(r, _C_WALL_CLOSED)
				draw_rect(r, Color(0.45, 0.35, 0.55, 0.40), false, 1.0)
				_draw_timer_arc(r, remaining, wall.phase_dur, Color(0.40, 0.40, 0.65, 0.50))
			_DynWall.State.OPENING:
				draw_rect(r, Color(_C_WALL_CLOSED.r, _C_WALL_CLOSED.g, _C_WALL_CLOSED.b,
					1.0 - progress))
				draw_rect(r, Color(0.20, 0.90, 0.30, 0.85), false, 2.0)
				_draw_timer_arc(r, remaining, wall.phase_dur, Color(0.20, 1.00, 0.30, 0.95))
...
func _draw_fragments() -> void:
	for f in _fragments:
		if f.collected:
			continue
		var alpha := 0.5 if f.collecting else 1.0
		draw_circle(f.pos, _FRAG_RADIUS, Color(0.95, 0.75, 0.25, alpha))
		draw_arc(f.pos, _FRAG_RADIUS + 3.0, 0.0, TAU, 8,
			Color(1.00, 0.90, 0.40, 0.40 * alpha), 1.5)
		if f.collecting:
			var p := clampf(f.collect_timer / _COLLECT_TIME, 0.0, 1.0)
			draw_arc(f.pos, _FRAG_RADIUS + 7.0, -PI * 0.5, -PI * 0.5 + TAU * p,
				32, Color(1.0, 1.0, 0.50, 0.95), 3.0)
...
func _draw_sentinels() -> void:
	for s in _sentinels:
		if not s.alive:
			continue
		draw_circle(s.pos, 11.0, Color(0.85, 0.18, 0.28))
		draw_arc(s.pos, 12.0, 0.0, TAU, 8, Color(1.00, 0.30, 0.40, 0.60), 2.0)
...
func _draw_player() -> void:
	...
	draw_circle(_player_pos, _PLAYER_RADIUS, c)
	draw_arc(_player_pos, _PLAYER_RADIUS + 2.0, 0.0, TAU, 12,
