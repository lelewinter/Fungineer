...
Unreal. Those were removed during the web port — Fungineer ships on PixiJS v8
+ Vite + TypeScript (frontend) and FastAPI + SQLite (backend). For stack work,
brief `gameplay-programmer`, `lead-programmer`, or `engine-programmer`
directly with the actual subsystem they should touch.

A `network-programmer` agent also existed in the template; the game is
single-player and save/load is plain HTTP, so it was removed too.
